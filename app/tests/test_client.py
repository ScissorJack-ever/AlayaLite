...
async def test_reset_persistence_collection(tmp_path, monkeypatch):
    ...
    async with create_app_client() as tc:
...
async def test_persistence_across_restart(tmp_path, monkeypatch):
    ...
    async with create_app_client() as tc2:
        ...
        resp = await tc2.post(
            "/api/v1/collection/delete", json={"collection_name": "restart_coll", "delete_on_disk": True}
        )
        assert resp.status_code == 200
    ...
    async with create_app_client() as tc3:
