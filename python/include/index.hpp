 ...
 public:
  ...
  auto load(const std::string &index_path, const std::string &data_path = std::string(),
    ...
    if constexpr (is_rabitq_space_v<SearchSpaceType>) {
      ...
      // when using pca, get_dim() can not return full dimensionality
      data_dim_ = search_space_->get_full_dim();
      search_job_ =
  ...
  auto fit(py::array_t<DataType> &vectors, uint32_t ef_construction, uint32_t num_threads) -> void {
    ...
    if constexpr (is_rabitq_space_v<SearchSpaceType>) {
      search_space_ = std::make_shared<SearchSpaceType>(params_.capacity_, data_dim_,
                                                        params_.metric_, params_.pca_);
