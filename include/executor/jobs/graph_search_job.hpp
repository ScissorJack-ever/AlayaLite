...
struct GraphSearchJob {
  ...
  void rabitq_supplement_result(SearchBuffer<DistanceType> &result_pool, HashBasedBooleanSet &vis,
    ...
    LOG_DEBUG("Failed to return enough knn, res_pool current size: {}", result_pool.size());
    ...
    LOG_DEBUG("Finished supplementing result, res_pool current size: {}", result_pool.size());
  }
  ...
  void rabitq_search_solo(const DataType *query, uint32_t k, IDType *ids, uint32_t ef) {
    ...
    while (search_pool.has_next()) {
      ...
      if (space_->use_pca_or_not()) {  // explicit rerank
        res_pool.insert(cur_node, space_->get_dist_func()(query, space_->get_data_by_id(cur_node),
                                                          space_->get_full_dim()));
      } else {  // implicit rerank
  ...
  auto rabitq_search([[maybe_unused]] const DataType *query, [[maybe_unused]] uint32_t k,
    ...
    LOG_INFO("In rabitq_search, use pca or not: {}",space_->use_pca_or_not());
    // init
    size_t degree_bound = RaBitQSpace<>::kDegreeBound;
    auto entry = space_->get_ep();
    mem_prefetch_l1(space_->get_data_by_id(entry), 10);
